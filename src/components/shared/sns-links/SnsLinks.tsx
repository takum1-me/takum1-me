...
type Variant = "twitter" | "instagram" | "github";

const snsData: { name: string; href: string; variant: Variant }[] = [
  {
    name: "X (Twitter)",
...
const renderIcon = (variant: Variant) => {
  switch (variant) {
    ...
    case "instagram":
      return (
        <img src={InstagramIcon} alt="" width={18} height={18} aria-hidden />
      );
    case "github":
      return (
        <svg width="18" height="18" viewBox="0 0 24 24" fill="currentColor">
...
export default function SnsLinks({ className = "" }: SnsLinksProps) {
  return (
    <div className={`sns-links ${className}`.trim()}>
      {snsData.map((sns) => (
        <a
          key={sns.name}
          href={sns.href}
          className={`sns-link sns-link--${sns.variant}`}
          target="_blank"
