...
export default function HoverIndicator({ items, onItemClick, className = "", showBackground = false, enableBlogFilter = false }: HoverIndicatorProps) {
  const [indicator, setIndicator] = useState<{ left: number; width: number; show: boolean }>({ 
    left: 0, 
    width: 0, 
    show: false 
  });
  ...
  const updateIndicator = (targetElement: HTMLElement, show: boolean = false) => {
  ...
  return (
    <div className={`hover-indicator-nav ${showBackground ? 'with-background' : 'no-background'} ${className}`} ref={navRef}>
      <span
      ...
      {items.map((item) => (
        <button
          ...
          onMouseLeave={() => setIndicator(prev => ({ ...prev, show: false }))}
          onClick={() => {
            if (enableBlogFilter) {
