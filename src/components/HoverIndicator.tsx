...
export default function HoverIndicator({
  ...
  const updateIndicator = useCallback((targetElement: HTMLButtonElement, show: boolean = false) => {
  ...
  const handleMouseEnter = useCallback((itemId: string, element: HTMLButtonElement | HTMLDivElement) => {
  ...
  const handleContainerMouseEnter = useCallback((itemId: string) => {
    setHoveredItem(itemId);
    const buttonElement = buttonRefs.current.get(itemId);
    if (buttonElement) {
      updateIndicator(buttonElement, true);
  ...
  const showNoBlogsMessage = useCallback((blogGrid: Element | null, categoryId: string) => {
    if (!blogGrid) return;
      ...
      {items.map((item) => (
        ...
        >
          <button
            ...
            onMouseEnter={() => handleMouseEnter(item.id, buttonRefs.current.get(item.id)!)}
            onMouseLeave={handleMouseLeave}
          >
