 ...
 */
export async function fetchNoteRSS(rssUrl: string = 'https://note.com/takum1_me_/rss'): Promise<NoteArticle[]> {
  try {
    ...
    for (const match of itemArray) {
      ...
      const description = descriptionMatch ? extractTextFromHTML(cleanXMLText(descriptionMatch[1])) : '';
      ...
      const thumbnailContentMatch = itemContent.match(/<media:thumbnail[^>]*>([\s\S]*?)<\/media:thumbnail>/i);
      ...
      if (!thumbnail) {
        const thumbnailUrlMatch = itemContent.match(/<media:thumbnail[^>]*url=["']([^"']+)["'][^>]*\/?>/i);
 ...
 */
export async function fetchArticleThumbnail(articleUrl: string): Promise<string | undefined> {
 ...
 */
export async function fetchThumbnailFromOGP(articleUrl: string): Promise<string | undefined> {
  try {
 ...
 * 複数の記事のサムネイルを並列で取得
 */
export async function fetchThumbnailsForArticles(articles: NoteArticle[]): Promise<NoteArticle[]> {
